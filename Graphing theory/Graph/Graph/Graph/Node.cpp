...
std::vector<node*>* Node::returnpath(std::string start, std::string end)
{
	...
	return findpath(startnode, endnode, path);
}

std::vector<node*>* Node::findpath(node* start, node* end, std::vector<node*>* path)
{
	...
	for (node* ni : start->connections)
	{
		...
		std::vector<node*>* temppath = findpath(ni, end, path);
		if (temppath == nullptr) {}
